...
/**
 * Enforces access to non-nutrition features.
 * Authenticates the request, then rejects "nutrition only" accounts — users who
 * are restricted to the nutrition calculator and must not reach expenses,
 * groceries, calendar, etc. (defense in depth behind the middleware + UI).
 * Returns { session } on success or { unauth: Response } (401/403) on failure.
 *
 * Usage:
